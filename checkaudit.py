...
def searchauditByItem(
    searchresult, make_choice, column_text, item_text, proc_text, pbc_text
):
...
def searchaudit(text, section_text, make_choice, industry_choice, top, flag):
    ...
    for query, query_embedding in zip(queries, query_embeddings):
        distances = scipy.spatial.distance.cdist(
            [query_embedding], sub_embeddings, "cosine"
        )[0]
...
def searchls2df(search_list, section_text, make_choice, choice, top, flag):
    ...
    for search in search_list:
        resuledf = searchaudit(search, section_text, make_choice, choice, top, flag)
